...
function fetchEvents(param, isInit) {
    ...
    $.ajax({
        ...
        // 件数分イベントカードを生成して追加する
        // 日付順にソートしてから出力（安全対策）
        filteredDatas.sort((a, b) => new Date(a.eventDate) - new Date(b.eventDate));

        let lastDateKey = null;
        for (let i = 0; i < filteredDatas.length && i < maxItems; i++) {
            const event = filteredDatas[i];
            // JST変換して日付キーを作成
            const dateObjForKey = new Date(event.eventDate);
            dateObjForKey.setHours(dateObjForKey.getHours() + 9);
            const dateKey = dateObjForKey.getFullYear() + '-' + ("00" + (dateObjForKey.getMonth() + 1)).slice(-2) + '-' + ("00" + dateObjForKey.getDate()).slice(-2);
            ...
            if (dateKey !== lastDateKey) {
                ...
                $('#event-columns').append(`
                        <div class="hero column col-12 py-2 my-2 bg-calendar">
                            <h3 class="my-2 py-2 text-bold text-light">${eventDate} ${youbi}</h3>
                        </div>
                `);
            }

            // 以下は既存のカード生成処理（インデックス名を i -> idx に変更）
            ...
            // イベントカード要素の追加
            $('#event-columns').append(
                `<div name="outer-card-upper-${i}" class="column col-6 col-lg-6 col-md-12 col-sm-12 col-xs-12 p-2 filter-item ${dataTag}" data-tag="${dataTag}">
